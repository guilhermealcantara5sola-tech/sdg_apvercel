...
class FivesimAPI:
    ...
    def get_balance(self):
        try:
            ...
            if res.status_code == 200: return float(res.json().get("balance", 0.0))
            return 0.0
        except Exception as e:
    ...
    def get_sms_code(self, activation_id):
        ...
        if res.status_code == 200:
            ...
            if sms_list: return sms_list[0].get("code") or sms_list[0].get("text")
        return None

    def finish_order(self, activation_id):
        requests.get(f"{self.url}/finish/{activation_id}", headers=self.headers, timeout=10)

    def cancel_order(self, activation_id):
        requests.get(f"{self.url}/cancel/{activation_id}", headers=self.headers, timeout=10)
...
class SmsActivateAPI:
    ...
    def get_balance(self):
        try:
            ...
            if res.status_code == 200 and "ACCESS_BALANCE:" in res.text: return float(res.text.split(":")[1])
            return 0.0
        except Exception as e:
    ...
    def get_sms_code(self, activation_id):
        ...
        res = requests.get(url, timeout=10)
        if res.status_code == 200 and "STATUS_OK:" in res.text: return res.text.split(":")[1]
        return None

    def finish_order(self, activation_id):
        requests.get(f"{self.url}?api_key={self.api_key}&action=setStatus&status=6&id={activation_id}", timeout=10)

    def cancel_order(self, activation_id):
        requests.get(f"{self.url}?api_key={self.api_key}&action=setStatus&status=8&id={activation_id}", timeout=10)
...
def generate_random_name():
    first_names = ["Aline", "Amanda", "Beatriz", "Bruna", "Camila", "Daniela", "Eduarda", "Fernanda", "Gabriela", "Isabela", "Juliana", "Larissa", "Luana", "Mariana", "Natalia", "Patrícia", "Rafaela", "Sandra", "Tatiane", "Vanessa", "Yasmin", "Alexandre", "Bruno", "Daniel", "Diego", "Eduardo", "Felipe", "Gabriel", "Guilherme", "Gustavo", "Igor", "João", "Lucas", "Mateus", "Otávio", "Pedro", "Rafael", "Rodrigo"]
    last_names = ["Silva", "Santos", "Oliveira", "Souza", "Rodrigues", "Ferreira", "Alves", "Pereira", "Lima", "Gomes", "Costa", "Ribeiro", "Martins", "Carvalho", "Rocha", "Melo", "Barbosa"]
...
def create_instagram_account(args, sms_api, account_idx):
    username = f"{args.username_prefix or 'sdg'}_{''.join(random.choices('abcdefghijklmnopqrstuvwxyz0123456789', k=5))}"
    ...
    if sms_api:
        ...
        log("Nenhuma chave de SMS informada. O script tentará usar e-mail.", "AVISO")
        phone_number = f"sdg_temp_{''.join(random.choices('abcdefghijklmnopqrstuvwxyz', k=8))}@mailto.plus"

    with sync_playwright() as p:
        browser = p.chromium.launch(headless=False, proxy={"server": args.proxy} if args.proxy else None)
        ...
        try:
            ...
            time.sleep(4)
            if "birthday" in page.url or page.query_selector("select"):
                ...
                if len(selects) >= 3:
                    ...
                    selects[2].select_option(label=str(random.randint(1990, 2005)))
                page.locator("button:has-text('Avançar')").first.click()
                time.sleep(5)
            if "challenge" in page.url or page.query_selector("iframe"):
                log("AVISO: Captcha detectado! Resolva manualmente.", "AVISO")
                for _ in range(36):
                    time.sleep(5)
                    if "challenge" not in page.url and not page.query_selector("iframe"): break
            if sms_api and activation_id:
                provider_name = "SMS-Activate.org" if isinstance(sms_api, SmsActivateAPI) else "5sim.net"
                log(f"Aguardando código no {provider_name}...", "SMS")
                sms_code = None
                for _ in range(36):
                ...
                if not sms_code:
                    ...
                    return False
                page.fill("input[name='email_confirmation_code']", sms_code)
                page.locator("button[type='submit']").first.click()
                time.sleep(10)
            success = any("instagram.com" in page.url for _ in range(6))
            if success:
                if sms_api and activation_id: sms_api.finish_order(activation_id)
                acc_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'accounts.json')
                data = json.load(open(acc_file, 'r')) if os.path.exists(acc_file) else {}
                data[username] = password
                json.dump(data, open(acc_file, 'w'), indent=2)
                browser.close()
                return True
            browser.close()
            return False
        except Exception:
            if sms_api and activation_id: sms_api.cancel_order(activation_id)
            browser.close()
            return False

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--sms-key")
    parser.add_argument("--country", default="brazil")
    parser.add_argument("--username-prefix", default="sdg")
    parser.add_argument("--password")
    parser.add_argument("--proxy")
    parser.add_argument("--count", type=int, default=1)
    args = parser.parse_args()
    sms_api = get_sms_api(args.sms_key) if args.sms_key else None
    for idx in range(1, args.count + 1):
        if create_instagram_account(args, sms_api, idx): log(f"Sucesso: conta {idx} criada.", "SUCESSO")
        time.sleep(15)
